...
from POSTagger.InitialTagger.InitialTagger import initializeCorpus, initializeSentence
from POSTagger.SCRDRlearner.Object import FWObject
from POSTagger.SCRDRlearner.SCRDRTree import SCRDRTree
from POSTagger.SCRDRlearner.SCRDRTreeLearner import SCRDRTreeLearner
from POSTagger.Utility.Config import NUMBER_OF_PROCESSES, THRESHOLD
from POSTagger.Utility.Utils import getWordTag, getRawText, readDictionary
from POSTagger.Utility.LexiconCreator import createLexicon

def unwrap_self_POSTagger(arg, **kwarg):
    return POSTagger.tagRawSentence(*arg, **kwarg)

class POSTagger(SCRDRTree):
    """
    POSTagger for a particular language
    """
    def __init__(self):
        self.root = None
    ...
    def tagRawCorpus(self, DICT, rawCorpusPath):
        ...
        taggedLines = pool.map(unwrap_self_POSTagger, zip([self] * len(lines), [DICT] * len(lines), lines))
...
def printHelp():
    print("\n===== Usage =====")  
    print('\n#1: To train POSTagger on a gold standard training corpus:')
    print('\npython POSTagger.py train PATH-TO-GOLD-STANDARD-TRAINING-CORPUS')
    print('\nExample: python POSTagger.py train ../data/goldTrain')
    print('\n#2: To use the trained model for POS tagging on a raw text corpus:')
    print('\npython POSTagger.py tag PATH-TO-TRAINED-MODEL PATH-TO-LEXICON PATH-TO-RAW-TEXT-CORPUS')
    print('\nExample: python POSTagger.py tag ../data/goldTrain.RDR ../data/goldTrain.DICT ../data/rawTest')
